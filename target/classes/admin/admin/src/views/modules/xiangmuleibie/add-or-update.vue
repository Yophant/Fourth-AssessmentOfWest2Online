<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="项目类别" prop="xiangmuleibie">
          <el-input v-model="ruleForm.xiangmuleibie" 
              placeholder="项目类别" clearable  :readonly="ro.xiangmuleibie"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiangmuleibie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiangmuleibieCrossAddOrUpdateFlag = false;
						 this.parent.search();
